...
A interface lista todo arquivo `*.py` na pasta da fase, em ordem alfabética. Não existe manifesto, validação prévia ou filtro para arquivos auxiliares. Todo arquivo listado deve expor `principal()` e ser seguro para importação dinâmica.
